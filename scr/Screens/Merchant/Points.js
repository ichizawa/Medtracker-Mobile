import {
  ...
  export default function Points({ navigation }) {
    ...
    const getTransasctions = () => {
      try {
        fetch(`${BASE_URL}merchant/merchant-points/${userInfo.details.user_id}`, {
          method: "GET",
          headers: {
          ...
          .then((res) => {
            ...
            if(statusCode === 200){
                console.log(data);
                setPointsData(data.merchantpoints);
            }
                ...
                pointsData.map((items, index) => {
                  ...
                  return (
                    <View key={index} style={{paddingHorizontal: 15, paddingVertical: 20, marginBottom: 10, flexDirection: 'row', borderRadius: 15, backgroundColor: '#fff', elevation: 5}}>
                      <View style={{flex: 1}}>
                        <Text style={{fontWeight: 'bold'}}>{items.status ? 'Recieved Points' : 'Cash-out'}</Text>
